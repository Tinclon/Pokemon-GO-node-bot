...
var location = configs.locations.newyork;
...
(function go() {
    ...
    pokeio.init(username, password, location, provider, function (err) {
        if (err) {
            console.log("Seems the Pokemon GO servers are offline. Trying again in 5 seconds");
            setTimeout(go, 5000);
            return;
        }
        ...
        pokeio.GetProfile(function (err, profile) {
            ...
            pokeio.Heartbeat(botTick(pokeio, location, go));
            //setInterval(function(){ pokeio.Heartbeat(botTick(pokeio, location)); }, 10000);
